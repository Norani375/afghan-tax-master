...
export const DeclarationsTable: React.FC = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const resp = await fetch('/api/tax-declarations');
      if (resp.ok) setDeclarations(await resp.json());
    } catch (err) { console.error('Failed to load declarations:', err); }
  ...
  const filteredCategories = Object.keys(grouped).filter((cat) => {
    ...
    if (searchTerm) return grouped[cat].some((d) => d.period.includes(searchTerm));
    return true;
  });
  ...
  const startEdit = (decl: TaxDeclaration) => {
    ...
    setEditData({ period: decl.period, filing_due_date: decl.filing_due_date || '', payment_due_date: decl.payment_due_date || '', submission_date: decl.submission_date || '', assessment: String(decl.assessment), paid: String(decl.paid), balance: String(decl.balance) });
  };
  ...
  const saveEdit = async (id: number) => {
    ...
    try {
      const resp = await fetch('/api/tax-declarations', {
        ...
        body: JSON.stringify({ id, period: editData.period, filing_due_date: editData.filing_due_date, payment_due_date: editData.payment_due_date, submission_date: editData.submission_date, assessment: Number(editData.assessment), paid: Number(editData.paid), balance: Number(editData.balance) }),
      });
      if (resp.ok) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await fetch(`/api/tax-declarations?id=${id}`, { method: 'DELETE' }); }
    catch (err) { console.error('Failed to delete:', err); load(); }
  };
  ...
  if (loading) {
    return (<div className="flex flex-col items-center justify-center py-16 gap-3"><span className="loading loading-spinner loading-lg text-primary" /><span className="text-base-content/60 text-sm">بارگذاری اظهارنامه‌ها...</span></div>);
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <label className="input input-bordered input-sm flex items-center gap-2 flex-1 min-w-[150px]">
          ...
          <input type="text" className="grow" placeholder="جستجو دوره..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </label>
        ...
        <button className="btn btn-primary btn-sm gap-1" onClick={() => setShowAddForm(true)}><Plus size={14} /> افزودن</button>
        ...
        <div className="card bg-base-200 border border-primary/30">
          <div className="card-body p-4 gap-3">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              <select className="select select-bordered select-sm col-span-2 sm:col-span-3" value={newDecl.tax_category} onChange={(e) => setNewDecl({ ...newDecl, tax_category: e.target.value })}>
                {Object.keys(grouped).length > 0 ? Object.keys(grouped).map((cat) => <option key={cat} value={cat}>{cat}</option>) : TAX_CATEGORIES.map((c) => <option key={c.id} value={c.label}>{c.label}</option>)}
              </select>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button className="btn btn-primary btn-sm" onClick={handleAdd} disabled={saving || !newDecl.period}>{saving ? <span className="loading loading-spinner loading-xs" /> : <Save size={14} />} ذخیره</button>
            </div>
          </div>
        </div>
      )}
      ...
      {filteredCategories.map((category) => {
        const items = grouped[category].filter((d) => !searchTerm || d.period.includes(searchTerm));
        ...
        return (
          <div key={category} className="card bg-base-200">
            <div className="flex items-center justify-between p-3 cursor-pointer hover:bg-base-300 rounded-t-2xl transition-colors" onClick={() => toggleCategory(category)}>
            ...
            {!isCollapsed && (
              <div className="overflow-x-auto px-2 pb-3">
                <table className="table table-sm table-zebra">
                  <thead><tr className="text-xs"><th>دوره</th><th>تاریخ ارایه اظهارنامه</th><th>تاریخ پرداخت</th><th>تاریخ ارایه شما</th><th className="text-left" dir="ltr">معینه</th><th className="text-left" dir="ltr">پرداخت شده</th><th className="text-left" dir="ltr">بلانس</th><th>عملیات</th></tr></thead>
                  <tbody>
                    {items.map((d) => (
                      <tr key={d.id}>
                        {editingRow === d.id ? (
                          <>
                            ...
                            <td><div className="flex gap-1"><button className="btn btn-success btn-xs" onClick={() => saveEdit(d.id)} disabled={saving}>{saving ? <span className="loading loading-spinner loading-xs" /> : <Save size={12} />}</button><button className="btn btn-ghost btn-xs" onClick={cancelEdit}><X size={12} /></button></div></td>
                          </>
                        ) : (
                          <>
                            ...
                            <td className={`font-mono text-xs text-left ${Number(d.balance) > 0 ? 'text-error' : 'text-success'}`} dir="ltr">{Number(d.balance) === 0 ? '✅ 0' : formatCurrency(Number(d.balance))}</td>
                            <td><div className="flex gap-1"><button className="btn btn-ghost btn-xs" onClick={() => startEdit(d)}><Edit3 size={12} /></button><button className="btn btn-ghost btn-xs text-error" onClick={() => handleDelete(d.id)}><Trash2 size={12} /></button></div></td>
                          </>
                        )}
                      </tr>
                    ))}
      ...
      {filteredCategories.length === 0 && (
        <div className="flex flex-col items-center py-12 gap-3 text-base-content/40"><FileText size={40} className="opacity-40" /><p className="text-sm">هیچ اظهارنامه‌ای یافت نشد</p></div>
      )}
    </div>
  );
};
