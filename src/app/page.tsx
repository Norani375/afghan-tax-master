...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/tax-records?limit=1')
      .then((r) => { if (r.ok) setDbConnected(true); else setDbConnected(false); })
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const resp = await fetch('/api/tax-records', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tax_type: selectedType,
          taxpayer_name: taxpayerName,
          gross_income: result.taxableAmount,
          deductions: 0,
          taxable_income: result.taxableAmount,
          tax_amount: result.totalTax,
          tax_rate: result.effectiveRate,
          period: selectedType === 'salary' ? 'ماهوار' : 'سالانه',
          notes: result.breakdown.map((b) => b.label).join(' | '),
        }),
      });
      if (resp.ok) {
  ...
  const handleLoadHistory = useCallback(async () => {
    ...
    try {
      const resp = await fetch('/api/tax-records?limit=50');
  ...
  const handleDeleteRecord = useCallback(async (id: number) => {
    ...
    try { await fetch(`/api/tax-records?id=${id}`, { method: 'DELETE' }); }
  ...
  return (
    <div className="min-h-screen bg-base-100 p-4 max-w-4xl mx-auto">
      ...
      <div className="tabs tabs-boxed bg-base-200 mb-6 justify-center flex-wrap">
        {tabs.map((tab) => (
          <button key={tab.id} className={`tab gap-2 ${activeTab === tab.id ? 'tab-active' : ''}`} onClick={() => setActiveTab(tab.id)}>
            {tab.icon}
            {tab.label}
          </button>
        ))}
